...
def load_fits_file(file_name, reverse_flag=False):
    ...
    # data = data['DATA'].reshape(
    naxis2, nsblk, npol, nchan, _ = data['DATA'].shape
    data = data['DATA'].reshape(naxis2*nsblk, npol, nchan)[:, :2, :]
...
#    model                     = SPPResNet(base_model, num_classes=2).to(device)
    ...
    for i in range(len(file_list) - 1):
        ...
        if raw_data.shape[0]  < comb_file_leng:
            print(raw_data.shape, (comb_file_leng-raw_data.shape[0], freq_reso))
            # raw_data          = np.append(raw_data, np.random.rand(comb_file_leng-raw_data.shape[0], 2, freq_reso) * raw_data.max() / 2, axis=0)
            raw_data          = np.append(raw_data, np.random.rand(comb_file_leng-raw_data.shape[0], 1, freq_reso) * raw_data.max() / 2, axis=0)
        # raw_data              = raw_data[:comb_file_leng, :, :]
        raw_data              = raw_data[:comb_file_leng, :]
        print(raw_data.shape, (raw_data.shape[0] // down_sampling_rate, down_sampling_rate, freq_reso))
        data                  = np.mean(raw_data.reshape(raw_data.shape[0] // down_sampling_rate, down_sampling_rate, 1, freq_reso), axis=(1, 2)).astype(np.float32)
        ...
        for block in blocks:
            ...
            np.save('{}-{:0>4d}.npy'.format(filename, block), data[block])
